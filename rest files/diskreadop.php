...
//if there are errors in the secrete and access key
      ...
      $response = $cw->get_metric_statistics($Namespace, "DiskReadOps", $start_time, $end_time, $period, $Statistics, "Count", $opts);
...
// Sort by key while maintaining association in order to have an oldest->newest sorted dataset
      $response = $cw->get_metric_statistics($Namespace, "DiskReadOps", "-7 day", "now", 86400, $Statistics, "Count", $opts);
     ...
     foreach ($d1 as $p) {
       ...
       ?>
                ...
                function drawChart() {
                    ...
                    data.addColumn('number', 'Diskread op');
                    for (var i = 0; i < 23; i++)
                    {
                        data.addRows([
                    ...
                    data1.addColumn('number', 'Diskread op');
                    for (var i = 0; i < 7; i++)
                    {
                        data1.addRows([
                    ...
                    // Set chart options
                    var options = {'title': 'DISKREAD OPERATIONS PER DAY',
                        'width': 400,
                        'height': 300,
                        'vAxis': {title: "diskread in bytes"},
                        'hAxis': {title: "time in h:m"}
                    };
                    var options2 = {'title': 'DISKREAD OPERATIONS PER MONTH',
                        'width': 500,
                        'height': 300,
                        'vAxis': {title: "diskread in bytes"},
            ...
            <div class="row">
                <div class="span2">
                    <ul class="nav nav-list">
                        ...
                        <li class="active"><a href="diskreadop.php">DiskReadOps</a></li>
