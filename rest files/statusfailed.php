...
//if there are errors in the secrete and access key
      ...
      $response = $cw->get_metric_statistics($Namespace, "StatusCheckFailed", $start_time, $end_time, $period, $Statistics, "Count", $opts);
...
// Sort by key while maintaining association in order to have an oldest->newest sorted dataset
      $response = $cw->get_metric_statistics($Namespace, "StatusCheckFailed", "-7 day", "now", 86400, $Statistics, "Count", $opts);
     ...
     foreach ($d1 as $p) {
       ...
       ?>
                ...
                function drawChart() {
                    ...
                    data.addColumn('number', 'status failed');
                    for (var i = 0; i < 23; i++)
                    {
                        data.addRows([
                    ...
                    data1.addColumn('number', 'status failed');
                    for (var i = 0; i < 7; i++)
                    {
                        data1.addRows([
                    ...
                    // Set chart options
                    var options = {'title': 'STATUS FAILED PER DAY',
                        'width': 400,
                        'height': 300,
                        'vAxis': {title: "status failed in count"},
                        'hAxis': {title: "time in h:m"}
                    };
                    var options2 = {'title': 'STATUS FAILED PER MONTH',
                        'width': 500,
                        'height': 300,
                        'vAxis': {title: "status failed in count"},
            ...
            <div class="row">
                <div class="span2">
                    <ul class="nav nav-list">
                        ...
                        <li class="active"><a href="statusfailed.php">StatusCheckFailed</a></li>
