...
//if there are errors in the secrete and access key
      ...
      $response = $cw->get_metric_statistics($Namespace, "DiskWriteBytes", $start_time, $end_time, $period, $Statistics, "Bytes", $opts);
...
// Sort by key while maintaining association in order to have an oldest->newest sorted dataset
      $response = $cw->get_metric_statistics($Namespace, "DiskWriteBytes", "-7 day", "now", 86400, $Statistics, "Bytes", $opts);
     ...
     foreach ($d1 as $p) {
       ...
       ?>
                ...
                function drawChart() {
                    ...
                    data.addColumn('number', 'Disk write');
                    for (var i = 0; i < 23; i++)
                    {
                        data.addRows([
                    ...
                    data1.addColumn('number', 'Disk write');
                    for (var i = 0; i < 7; i++)
                    {
                        data1.addRows([
                    ...
                    // Set chart options
                    var options = {'title': 'DISK WRITE PER DAY',
                        'width': 400,
                        'height': 300,
                        'vAxis': {title: "disk write in Bytes"},
                        'hAxis': {title: "time in h:m"}
                    };
                    var options2 = {'title': 'DISK WRITE PER MONTH',
                        'width': 500,
                        'height': 300,
                        'vAxis': {title: "Cpu Utilization in Bytes"},
                ...
                <div class="span2">
                    <ul class="nav nav-list">
                        ...
                        <li><a href="diskreadop.php">DiskReadOps</a></li>
                         <li class="active"><a href="diskwrite.php">DiskWriteBytes</a></li>
